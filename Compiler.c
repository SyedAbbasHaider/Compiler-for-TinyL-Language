...
static void assign()
{
	...
	char var = token;
	next_token();	//go to the next token
	...
	next_token();	//go to the next token
	reg = expr();	//call expr function to get the true value of expresion 
	CodeGen(STORE, var, reg, EMPTY_FIELD);	//Assign the true value by calling codegen(STORE)
}

static void read()
{
	/* YOUR CODE GOES HERE --DONE--*/
	if(token == '%'){	//this is my own modification, I think we should check for read sign.
		next_token();
...
static void stmt()
{
	...
	switch(token){
		...
		default:
		ERROR("Character is not recognized %c \n",token);
		exit(EXIT_FAILURE);
	}
}
